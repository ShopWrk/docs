---
title: "Review Stripe billing events"
description: "Review the 50 most recent Stripe subscription-billing events ShopWrk has processed, understand their statuses, and replay failed ones after fixing the cause."
sidebarTitle: "Stripe billing events"
---

ShopWrk receives webhook events from Stripe whenever something happens with your subscription billing: a plan change processes, a renewal charge succeeds, or a payment fails. The webhook log at **Settings → Webhooks** shows the 50 most recent events so you can see what has been processed and investigate anything that did not succeed.

<Note>
  This page covers ShopWrk's own billing connection to Stripe. It does not show customer payments, which run through [ShopWrk Pay](/integrations/shopwrk-pay). If no events appear yet, the first will arrive when your shop's plan or billing status changes.
</Note>
