...
A warranty profile records the manufacturer warranty attached to a service: registration method, registration deadline, coverage details, and inspection requirements. Set it up once under **Settings → Warranties**, then use it for eligible job services.
...
After registration, ShopWrk generates a warranty certificate summarizing the coverage, and the customer can access it through their [customer portal](/workflows/customer-portal). A tokenized PDF download route also exists for sharing. Check the sharing option shown on the warranty record before promising a customer login-free access.
...
<Steps>
  ...
  <Step title="Update the claim status">
    A new claim starts as **Submitted**. Move it to **Under Review**, then choose **Approved** or **Denied** with resolution notes. After approved corrective work finishes, mark the claim **Completed**.
  </Step>
